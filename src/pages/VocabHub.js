...
const VocabHub = () => {
  ...
  <MainLayout>
    <div className="space-y-6 pb-32">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 bg-white/40 dark:bg-slate-900/40 backdrop-blur-3xl p-4 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-emerald-500/10 shadow-xl">
        <div className="flex items-center gap-3 md:gap-5">
          <div className="w-12 h-12 md:w-16 md:h-16 bg-emerald-600 rounded-2xl md:rounded-3xl flex items-center justify-center shadow-lg shadow-emerald-500/10 -rotate-2 md:-rotate-3 hover:rotate-0 transition-transform">
            <BookOpen className="w-6 h-6 md:w-8 md:h-8 text-white" />
          </div>
          <div>
            <h1 className="text-xl md:text-3xl font-black italic tracking-tighter uppercase text-slate-900 dark:text-white leading-none">Vocabulary Hub</h1>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[8px] md:text-[10px] font-black text-emerald-500 tracking-[0.2em] md:tracking-[0.3em] uppercase">{selectedLevel} Lexicon Mastery</span>
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-xl flex flex-1 md:flex-none">
            <button
              className={`flex-1 md:flex-none px-4 py-2 rounded-lg font-black italic transition-all ${selectedLevel === 'N5' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
              onClick={() => { setSelectedLevel('N5'); setSelectedCategory('all'); setDisplayCount(50); }}
            >
              N5
            </button>
            <button
              className={`flex-1 md:flex-none px-4 py-2 rounded-lg font-black italic transition-all ${selectedLevel === 'N4' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
              onClick={() => { setSelectedLevel('N4'); setSelectedCategory('all'); setDisplayCount(50); }}
            >
              N4
            </button>
          </div>
          <Link to="/quiz/vocab" className="flex-1 md:flex-none">
            <Button className="h-12 w-full md:w-auto px-6 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black italic tracking-tight shadow-xl shadow-emerald-500/10 group">
